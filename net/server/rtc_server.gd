...
class Lobby extends RefCounted:
	...
	func join(peer: Peer) -> bool:
		...
		peer.send(Msg.Message.ID, peer.id, JSON.stringify({
			"host": peer.id == host,
			"mesh": mesh
		}))
		for p: Peer in peers.values():
			...
			if not mesh and p.id != host:
				...
				continue
			p.send(Msg.Message.PEER_CONNECT, peer.id)
			peer.send(Msg.Message.PEER_CONNECT, (1 if p.id == host else p.id))
		peers[peer.id] = peer
		return true
	...
	func leave(peer: Peer) -> bool:
		...
		for p: Peer in peers.values():
			...
			if close:
				...
				p.send(Msg.Message.PEER_DISCONNECT, peer.id)
...
func _ready():
	listen(7777)
	print("RTC server running on 7777")
...
func _join_lobby(peer: Peer, lobby: String, mesh: bool) -> bool:
	...
	peer.send(Msg.Message.JOIN, 0, lobby)
...
func _parse_msg(peer: Peer) -> bool:
	...
	if msg.type in [Msg.Message.OFFER, Msg.Message.ANSWER, Msg.Message.CANDIDATE]:
